...
// Hàm ánh x? b?ng ch? thay th? cho các ký t? c? th?
unordered_map<char, char> generateSubstitutionCipher(const string &originalChars, const string &substituteChars)
{
    ...
    // Ð?m b?o d? dài c?a hai chu?i b?ng nhau
...
// Hàm ánh x? b?n rõ v?i ánh x? dã t?o
...
void mapCipherTextToPlainText(const unordered_map<char, char> &cipher, const string &ciphertext)
{
    ...
    for (char c : ciphertext)
    {
        // Tìm ký t? b?n rõ tuong ?ng trong ánh x?
...
bool containsVietnameseDiacritics(const std::string& str) {
    // Duy?t qua t?ng kí t? trong chu?i UTF-8
    for (size_t i = 0; i < str.size(); ++i) {
        // N?u là kí t? ti?ng Vi?t có d?u
        if ((str[i] & 0xC0) == 0x80) {
            return true;
        }
    }
    return false; // N?u không có kí t? ti?ng Vi?t có d?u, tr? v? false
}
